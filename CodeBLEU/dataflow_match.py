...
def corpus_dataflow_match(references, candidates, lang):   
    LANGUAGE = Language('/home/bt2/18CS10067/IR/IR_project_JESR_NCS/datautils/parser/my-languages.so', lang)
    parser = Parser()
    ...
    for i in range(len(candidates)):
        ...
        for reference in references_sample:
            try: candidate=remove_comments_and_docstrings(candidate,lang)
            except: pass #'java')
            try: reference=remove_comments_and_docstrings(reference,lang)
            except: pass #'java')
